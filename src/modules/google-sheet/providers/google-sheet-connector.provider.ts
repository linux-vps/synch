...
@Injectable()
export class GoogleSheetConnectorProvider {
    private authClient: OAuth2Client;
    private _loadedSpreadsheet: sheets_v4.Schema$Spreadsheet;
    private loadedSheet: sheets_v4.Schema$Sheet;
  
    constructor(private readonly authService: GoogleAuthProvider) {
      // Không gọi initialize() ở đây
    }

    /**
     *
     * @returns {Promise<OAuth2Client>} OAuth2 Client.
     */
    private async getAuthClient(): Promise<OAuth2Client> {
        if (!this.authClient) {
            this.authClient = await this.authService.getClient();
        }
    ...
    getLoadedSpreadSheet(): sheets_v4.Schema$Spreadsheet {
     ...
     // Tải bảng tính từ Google Sheets API
    async loadSpreadSheet(spreadsheetId: string): Promise<sheets_v4.Schema$Spreadsheet> {
        ...
        this._loadedSpreadsheet = res.data;  // Lưu bảng tính đã tải
    ...
    // Chuyển đổi ký tự cột (A, B, ...) thành số
